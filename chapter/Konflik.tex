...
\end{enumerate}
Ada pun cara lain jika Anda mengalami kejadian "\textif{Unable to Merge}" dan Anda kebingungan karena Anda sudah \textif{push} tetapi pada saat \textif{pull request} Anda tidak bisa melakukannya, cara ini mungkin dapat membantu Anda agar bisa melakukan \textif{pull request} (terutama file konflik nya ada di file PDF, akan sangat berguna menggunakan cara ini).
\begin{enumerate}
\item \textif{backup} dahulu yang sudah anda kerjakan. Tidak perlu \textif{backup} keseluruhan repo, cukup \textif{file} yang anda kerjakan,
\item Buka repo asal (misal BukuInformatika/git) dari pekerjaan yang Anda kerjakan (bukan repo hasil \textif{fork} kita sendiri),
\item \textif{Download ZIP, Extract} ke direktori lokal pekerjaan repo Anda,
\item Lakukan sinkronisasi dengan Repo asal untuk memastikan apakah masih ada \textif{error} ketika kita melakukan \textif{merge},
\item Jika tidak ada \textif{error}, maka, masukkan lagi pekerjaan yang sudah kita \textif{backup} tadi,
\item \textif{add file, commit, push},
\item Coba lagi melakukan \textif{Pull Request}, maka hasilnya akan menjadi ceklis hijau.
\end{enumerate}